...
기존의 단순 주입 방식에서 메뉴 기반의 turn-end 시스템으로 전환하여, AI가 작업 완료 여부를 명시적으로 선택할 수 있도록 개선했습니다.

### Work-Complete Signal

OpenCode와 OMX 공통으로 `.context/.work-complete` 파일을 사용하여 작업 완료 신호를 처리합니다.

- OpenCode: 파일의 mtime을 비교하여 최신 상태를 확인합니다.
- OMX: turn_id를 비교하여 해당 턴의 완료 여부를 검증합니다.

### OMX Specifics

OMX 환경에서는 `explicit-stop`이 `followup`이나 `duplicate suppression`보다 우선순위를 가집니다. 이를 통해 `pending followup state`를 정리하고 불필요한 루프를 방지합니다.

관련 노트: [[synthetic-message-injection.md]]
...
- `config.jsonc` preserve 정책: 설정 파일은 사용자의 커스텀 설정을 보존합니다.
